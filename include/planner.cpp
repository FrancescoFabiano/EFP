...
CKripke*
Planner::search (CKripke* cur, CKripkeQueue* queue)
{
...
CKripke*
Planner::searchwithheur (CKripke* cur, Priority_Queue* queue)
{
  KActionList::iterator it;
  CKripke* out;
  for (it = ontic.begin (); it != ontic.end (); it++)
  ...
  for (it_st = sl->begin (); it_st != sl->end (); it_st++)
    {
      Kstate* temp = &(*it_st);
      computeLiteral (temp);
      if (ini.musthave.size () == 0 || temp->entail_fluform (ini.musthave))
        { //add only states that satisfies C(phi)
          temp->set_id (ids);
...
FluentFormula*
Planner::and2ff (FluentFormula* flu1, FluentFormula* flu2)
{
...
bool
Planner::is_consistent (Fluents f1, Fluents f2)
{
  Fluents::iterator it;
  Fluent nl;
...
void
Planner::computeLiteral (Kstate* s)
{
...
FluentFormula*
Planner::convert (StringList2 list)
{
...
Literals
Planner::ground (const StringList* x) const
{
...
Literal
Planner::ground (const string& x) const
{
...
StringList
Planner::convert (const Literals& x) const
{
  StringList y;
...
string
Planner::convert (const Literal& x) const
{
...
Agents
Planner::convert (StringList list)
{
  Agents output;
  StringList::iterator it;
...
/* printing functions */
void
Planner::print (const Literal& l) const
{
  cout << convert (l);
}

void
Planner::print (const Literals& x) const
{
  Literals::iterator it;
  bool comma = false;
  for (it = x.begin (); it != x.end (); it++)
...
void
Planner::print_statistics () const
{
...
KAction*
Planner::add_ontic (const string str, PropositionType pt)
{
...
Literals
Planner::negate (const Literals& x) const
{
...
void
Planner::print_fluform (StringList2 in)
{
...
void
Planner::print_fluform (FluentFormula in)
{
  FluentFormula::iterator it1;
...
void
Planner::print_literals (const Literals& ls) const
{
  Literals::iterator it;
  bool begin = true;
...
void
Planner::print_ontic (KAction* k) { }

bool
Planner::check_Agents (Agent ag, Agents list)
{
  Agents::iterator it;
...
bool
Planner::is_executable (Kripke k, KAction a) const
{
...
bool
Planner::is_executable1 (Kripke k, KAction a) const
{
  Nodes::const_iterator it;
  for (it = a.get_execs ()->begin (); it != a.get_execs ()->end (); it++)
    {
...
Kripke*
Planner::execute_ann (Kripke k, KAction a)
{
  int oldpId = k.get_id ();
...
Kripke*
Planner::execute_sensing (Kripke k, KAction a)
{
  int oldpId = k.get_id ();
...
bool
Planner::isSameSatisfyFF (Kstate k1, Kstate k2, FluentFormula check) const
{
...
bool
Planner::isSameInterpret (Kstate k1, Kstate k2, Literals check) const
{
  Literals::iterator it;
...
Kripke*
Planner::execute_ontic (Kripke k, KAction a)
{
  int oldpId = k.get_id ();
...
bool
Planner::check_lits (Literal a, Literals list)
{
  Literals::iterator it;
...
Kripke*
Planner::update_kripke (Kripke k)
{
...
void
Planner::update_node (BFNode* node)
{
...
AgentSet
Planner::get_fd (const KAction& act, Kripke k) const
{
...
AgentSet
Planner::get_pd (const KAction& act, Kripke k) const
{
  AgentSet out = AgentSet ();
  Agent_FF_list::iterator ia;
...
AgentSet
Planner::get_od (const KAction& act, Kripke k) const
{
  AgentSet ags = m_agents;
...
Literals
Planner::compute_effect (KAction* a, Kripke* k)
{
...
Kstate*
Planner::comp_nextstate (Kstate prev, Literals lits)
{
  ...
  for (it = lits.begin (); it != lits.end (); it++)
    {
      if ((*it) % 2 == 1) // this literal is negation
        ...
        { // this literal is not a negation
          lit.erase ((*it) + 1);
          lit.insert (*it);
        }
    }
...
Kripke*
Planner::merge_kripkes (Kripke* k1, Kripke* k2, map<int, int>* saved)
{
...
EdgeList
Planner::updateEdgesListId (EdgeList edgs, map<int, int> maps)
{
  EdgeList out;
...
void
CKripke::set_result (Kripke* ik)
{
  k = ik;
};

void
CKripke::add_path (string name)
{
  path.push_back (name);
}
...
int
CKripke::lengthOfPlanningGraph () //return the maximal length of Planning Graphs; each of which is a PlanningGraph of a Kripkek in ks
{
  int out = 0;
  vector<Kripke*>::iterator itk;
  for (itk = ks.begin (); itk != ks.end (); itk++)
...
void
CKripke::set_path (vector<string> st)
{
  path = st;
}

void
CKripke::set_old (CKripke* o)
{
  old = o;
}
