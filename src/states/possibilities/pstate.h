...
private:
    ...
    void calculate_no_good_edges(const finitary_theory &theory, pedge_map &no_goods);

    void filter_no_good_edges(const domain &domain, pedge_map &no_goods);
    ...
    bool entails(const formula &formula, const pworld &world) const;

    void update(const pstate *previous, const action *action);

    /** Function that calculates all the reachable \ref pworld.
     * 
     * @param[in] world: the current \ref pworld.
     * @param[in] reached_worlds: the set of all the reached \ref pworld.
     * @param[in] adj_list: the adjacency list of thr \ref pstate.*/
    void get_all_reachable_worlds_edges(const pworld *world, pworld_set & reached_worlds, pedge_map & reached_edges) const;

    /** Function that cleares all the unreachable \ref pworld of *this*
     *
     * @param[in] adj_list: the adjacency list of thr \ref pstate.*/
     ...
     * 
     * @param[in] pworld_vec: the vector of all \ref pworld.
     * 
     * @return the automaton equivalent to *this*.*/
    //     * @param[in] index_map: a map that associates a unique id to each \ref pworld.
    //     * @param[in] compact_indices: we associate to the \ref pworld numerical ids a second kind of numerical ids. These new ids are consecutive numbers (hence, they are compact).
    automaton pstate_to_automaton(std::vector<pworld*> & pworld_vec, const std::map<agent, bis_label> & agent_to_label) const;
    /** \brief Function that transforms the given automaton into an equivalent \ref pstate.
     * 
     * @param[in] a: the automaton to transform.
     * @param[in] pworld_vec: the vector of all \ref pworld.
     * 
     * @return the \ref pstate.*/
    void automaton_to_pstate(const automaton & a, const std::vector<pworld*> & pworld_vec, const std::map<bis_label, agent> & label_to_agent);

public:
    pstate();

    pstate(const domain &domain, const finitary_theory &theory);

    pstate(const pstate *previous, const action *action);
    /** \brief Getter of the field \ref m_worlds.
     *
     * @return: the value of \ref m_worlds.*/
    const pworld_set &get_worlds() const;
    /** \brief Getter of the field \ref m_information_state.
     *
     * @return: the value of \ref m_information_state.*/
    const pedge_map &get_edges() const;
    /** \brief Getter of the field \ref m_pointed.
     *
     * @return: the value of \ref m_pointed.*/
    const pworld *get_pointed() const;
...
//    bool operator=(const pstate & to_copy);
     ...
     *
     * The ordering is based on pworlds and the pedges and pointed.
     *
     * @param[in] to_compare: the \ref pstate to check for odering.
     *
...
//    pstate compute_succ(const action & act) const;
    ...
    /** \brief Function that sets *this* as the mimimum \ref pstate that is bisimilar to the current one.
     *
    ...
    //    void debug_print(const pstate & to_compare);
    //    void automaton_to_pstate_debug(const automaton & a, std::vector<pworld*> & pworld_vec);
};
...
//     * 
//     * @param[in] ag: the label of the \ref pedge that the function follows to check the transitivity.
...
//     * 
//     * \todo self-loop?*/
//    pworld_set get_B_reachable_worlds(const agent &ag, const pworld_ptr &world) const;
//
//    /** \brief Function that returns all the reachable \ref pworld given the \ref agent and the staring \ref pworld.
//     * 
...
//     * 
//     * \todo self-loop?*/
//    bool get_B_reachable_worlds_recursive(const agent &ag, const pworld_ptr &world, pworld_set &ret) const;
//
...
//     * 
//     * @param[in] ags: the set of label of the \ref pedge that the function follows to check the transitivity.
//     * @param[in] world: the pointers to the set of \ref pworld where to start to check the entailment.
//     *
//     * @return a set of pointers to all the reachable worlds.
//     *
//     * 
//     * \todo self-loop?*/
//    pworld_set get_E_reachable_worlds(const agent_set &ags, const pworld_ptr &world) const;
//
//    /** \brief Function that returns all the reachable \ref pworld given a set of \ref agent and the staring \ref pworld.
//     * 
//     * This function modify the parameter so it's easy to check if a fixed point is reached.
...
//     * 
//     * \todo self-loop?*/
//    bool get_E_reachable_worlds_recoursive(const agent_set &ags, const pworld_set &worlds,
//                                           pworld_set &reached) const;
//
...
//     * 
//     * \todo self-loop?*/
//    pworld_set get_D_reachable_worlds(const agent_set &ags, const pworld_ptr &world) const;
//
...
//     * 
//     * @param[in] ags: the set of label of the \ref pedge that the function follows to check the *Common Knowledge* reachability.
//     * @param[in] world: the pointers to the set of \ref pworld where to start to check the entailment.
//     *
//     * @return a set of pointers to all the reachable worlds.
//     * 
//     * \todo self-loop?*/
//    pworld_set get_C_reachable_worlds(const agent_set &ags, const pworld_ptr &world) const;
//
...
//     *         -# create a \ref pedge between all this \ref pworld (transitively closed) labeled with ag;
//     *         -# link all the classes of \ref pworld with an ongoing \ref pedge labeled with the ag of the reaching class.
//     * 
//     * @see pworld, pedge and initially.*/
...
//
//    /** \brief Function used to build the \ref pedge of the initial \ref pstate.
//     *  
...
//
//    void get_all_reachable_worlds_edges(const pworld_ptr &world, pworld_set &reached_worlds,
//                                        pedge_map &reached_edges) const;
//
//    void clean_unreachable_pworlds();
//
//
...
//
//    automaton
//    pstate_to_automaton(std::vector<pworld_ptr> &pworld_vec, const std::map<agent, bis_label> &agent_to_label) const;
//
//    void automaton_to_pstate(const automaton &a, const std::vector<pworld_ptr> &pworld_vec,
//                             const std::map<bis_label, agent> &label_to_agent);
//
...
//    void set_worlds(const pworld_set &to_set);
//
...
//    void set_beliefs(const pedge_map &to_set);
//
...
//    const pedge_map &get_beliefs() const;
//
...
//     *
//     * \todo self-loop?*/
//    bool entails(const belief_formula &to_check, const pworld_set &worlds) const;
//
...
//    pworld_ptr add_rep_world(const pworld &to_add, unsigned short repetition);
//
...
//    bool check_properties(const agent_set &fully, const agent_set &partially, const fluent_formula &effects,
//                          const pstate &updated) const;
//
...
//    * @param[in] start: the world to set as pointed to check the entailment.
//    * @return the effects that are feasible in *this* with \p start as pointed world*.*/
//    agent_set get_agents_if_entailed(const mal_observability_map &map, const pworld_ptr &start) const;
//
...
//    pworld_ptr
//    execute_ontic_helper(const action &act, pstate &ret, const pworld_ptr &current_pw, transition_map &calculated,
//                         agent_set &oblivious_obs_agents) const;
//
//    /** \brief Function that recursively calculates the \ref pworld resulting from the transition function.
//     *
...
//     * @return the \ref pworld resulting from the application of the transition function on "current_pw".*/
//    pworld_ptr
//    execute_sensing_announcement_helper(const fluent_formula &effects, pstate &ret, const pworld_ptr &current_pw,
//                                        transition_map &calculated, agent_set &partially_obs_agents,
//                                        agent_set &oblivious_obs_agents, bool previous_entailment) const;
//
//public:
