...
class CondEff(object):
    ...
    def project(self, ag, akprops):
        ...
        if isinstance(self.eff, Possible):
            return (False, CondEff(condp, condn, neg(self.eff.rml), self.ma_cond))
        elif isinstance(self.eff, Belief):
            return (True, CondEff(condp, condn, self.eff.rml, self.ma_cond))
        elif self.eff.is_ak(akprops):
            return (True, CondEff(condp, condn, self.eff, self.ma_cond))
        else:
